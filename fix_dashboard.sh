...
# Script de correction des erreurs dashboard
echo "=== CORRECTION DES ERREURS DASHBOARD ==="

# 1. Vérifier les routes admin
echo "1. Vérification des routes admin..."
php artisan route:list --name=admin | head -20
...
try {
    \$admin = User::where('role', 'admin')->orWhere('is_admin', 1)->first();
    if (!\$admin) {
        \$admin = User::create([
            'name' => 'Admin',
        ...
        echo 'Utilisateur admin créé: admin@admin.com (mot de passe: admin123)' . PHP_EOL;
    } else {
        echo 'Admin existe déjà: ' . \$admin->email . PHP_EOL;
    }
} catch (Exception \$e) {
